...
def global_evaluation(model, tokenizer, prompter, dev_data_path):
    ...
    acc_count_dict = dict.fromkeys(data_class, 0)  #设置每个类别的问题答对的数量，总数，准确率字典
    ...
    for data_point in tqdm(test_set):
        ...
        tgt_ans_idx = target.replace('The answer is: ','').split('. ')[0] #从答案中提取索引（A、B、C...）
        tgt_ans = target.replace('The answer is: ','').split('. ')[1] #从答案中提取内容
        ...
        with torch.autocast("cuda"):
            ...
            with torch.no_grad():
                ...
                generation_output = model.generate(
                    input_ids=input,
                    generation_config=sampling, #文本生成策略
                    return_dict_in_generate=True, #以字典形式返回结果
                    output_scores=True, #返回每个生成步骤中所有词汇得分
                    max_new_tokens=max_new_token, #模型生成答案的最大长度
                    pad_token_id=tokenizer.eos_token_id #将填充符（pad token）设置为结束符（eos token）
                )
