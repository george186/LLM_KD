...
from load_dataset import DollyDataset
...
def fl_finetune(
        ...
        cilent_lora_target_modules: List[str] = [
    ...
    if int(os.environ.get("LOCAL_RANK", 0)) == 0:
        print(
            ...
            f"client_lora_target_modules: {cilent_lora_target_modules}\n"
    ...
    config_client = LoraConfig(
                r = lora_r, 
                lora_alpha = lora_alpha, 
                target_modules = cilent_lora_target_modules,
                lora_dropout = lora_dropout,
                bias = "none",
                task_type = "CAUSAL_LM",
            )
    ...
    # 准备全局llama模型
    print("\nInitializing Llama...")
    server = GeneralServer(llama_path, output_dir, config_server)
    ...
    for client_id in selected_clients_set:
        clients.append(GeneralClient(client_id, model_path, output_dir, config_client, 4096))
